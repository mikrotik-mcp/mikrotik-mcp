...
| **[Offline Simulator](./simulator.md)**             | Trace a hypothetical packet through NAT, routing and firewall against a config snapshot — no device touched; UNKNOWN rather than a guess.                                                     |
| **[Policy-as-Code](./policy-as-code.md)**           | Author your own compliance rules in YAML and evaluate them against a config snapshot — offline, read-only, CI-able, with Markdown/JSON/SARIF output.                                          |
