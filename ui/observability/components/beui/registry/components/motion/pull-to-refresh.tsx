...
import {
  ...
  SPRING_PANEL,
  SPRING_SWAP,
} from "@/components/beui/registry/lib/ease";
import { capturePointer, TOUCH_GESTURE_CONTENT_CLASS } from "@/components/beui/registry/lib/touch";
...
export interface PullToRefreshProps {
  ...
  /** Limit gestures to an explicit handle, leaving forms and table selection alone. */
  handleOnly?: boolean;
...
export function PullToRefresh({
  ...
  }, [onRefresh, onRefreshError, restingDistance, setStatus, settle]);
  ...
  const canStart = useCallback(
    (target: EventTarget | null) => {
      ...
      if (
        ...
        return false;
      return !handleOnly || Boolean(target.closest("[data-refresh-handle]"));
    },
    [handleOnly],
  );
  ...
  useEffect(() => {
    ...
    root.addEventListener("touchcancel", cancelPull);

    return () => {
      root.removeEventListener("touchstart", onTouchStart);
      root.removeEventListener("touchmove", onTouchMove);
      root.removeEventListener("touchend", onTouchEnd);
      root.removeEventListener("touchcancel", cancelPull);
    };
  }, [finishPull, updatePull, canStart, cancelPull]);
  ...
  return (
    <section
      ...
      onPointerCancel={(event) => {
        if (gestureRef.current.pointerId === event.pointerId) cancelPull();
      }}
      className={cn(
        ...
        !handleOnly && TOUCH_GESTURE_CONTENT_CLASS,
