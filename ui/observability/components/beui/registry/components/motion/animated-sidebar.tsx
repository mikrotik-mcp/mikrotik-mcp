...
import { SharedLayoutBg } from "@/components/beui/registry/components/motion/shared-layout-bg";
import {
  EASE_DRAWER,
  EASE_OUT,
  SPRING_LAYOUT,
...
function MobileSidebar({
  ...
  const focusPanel = useCallback(() => {
    ...
    const first = [...panel.querySelectorAll<HTMLElement>(FOCUSABLE_SELECTOR)].find(
      (node) => node.getClientRects().length > 0 && !node.closest("[inert], [hidden]"),
    );
  ...
  return createPortal(
    <div
      className={cn(
        "pointer-events-none fixed left-0 top-0 z-[45] size-0 min-[900px]:hidden",
  ...
  (
    ...
    return (
      <motion.aside
        ...
        style={style}
        className={cn(
          "group/sidebar relative hidden h-auto shrink-0 min-[900px]:block will-change-[width]",
          "peer",
...
export const AnimatedSidebarMenuSub = forwardRef<HTMLUListElement, AnimatedSidebarMenuSubProps>(
  ({ open, children, className, ...props }, forwardedRef) => {
    const context = useAnimatedSidebar();
    const panel = useAnimatedSidebarPanel();
...
export interface AnimatedSidebarMenuButtonProps {
  ...
  onSelect?: () => void;
  className?: string;
  /** Distinguish an active shortcut from the same item in its category. */
...
export function AnimatedSidebarMenuButton({
  ...
  const content = (
    <>
      {isActive ? (
        <motion.span
          layoutId={activeLayoutId ? `${context.layoutId}-${activeLayoutId}` : context.layoutId}
