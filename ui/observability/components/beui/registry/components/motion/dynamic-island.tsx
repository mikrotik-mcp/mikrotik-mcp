...
function Slot({
  ...
  const bottom = useContext(IslandContext)?.anchor === "bottom";
  return (
    <motion.div
      ref={ref}
      key={keyId}
      initial={
        reduce
          ...
          : { opacity: 0, scale: bottom ? 1 : 0.9, y: bottom ? 0 : -8, filter: "blur(5px)" }
      }
      animate={
        reduce
      ...
      transition={
        reduce ? { duration: 0.1 } : bottom ? { duration: 0.16, ease: EASE_OUT } : CONTENT_SPRING
      }
      // Anchored to the pill line: content unfurls downward out of it and is
      // sucked back up into it.
      style={{ transformOrigin: bottom ? "bottom center" : "top center" }}
...
export interface DynamicIslandProps {
  ...
  children?: ReactNode;
  className?: string;
  /** Interactive dashboards announce a short summary separately from controls. */
  announce?: boolean;
  /** Fixed bottom companions must resize upward, keeping the pill baseline still. */
  anchor?: "top" | "bottom";
}
...
export function DynamicIsland({
  ...
  const contextValue = useMemo(() => ({ view, anchor }), [view, anchor]);
  ...
  return (
    <IslandContext.Provider value={contextValue}>
      <motion.div
        ...
        className={cn(
          "relative inline-flex justify-center overflow-hidden",
          anchor === "bottom" ? "items-end" : "items-start",
      ...
      >
        ...
        <div ref={sizerRef} className="relative w-max shrink-0">
